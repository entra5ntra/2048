...
// 初始化游戏状态
init() {
    // 尝试从本地恢复游戏状态，如果成功则跳过初始化流程
    if (this.loadGameState()) {
        return; // 已经加载成功，不需要重新开始
    }

    // 如果无法恢复，才执行初始化逻辑
    this.grid = this.createGrid(this.gridSize); // 创建空网格
    this.score = 0;
    ...
    this.previousState = null; // 清除撤销历史
}
...
saveGameState() {
    const state = {
        // 把每个方块的位置和值记录下来（DOM元素不能存储）
        grid: this.grid.map(row => row.map(cell => cell ? { x: cell.x, y: cell.y, value: cell.value } : null)),
        score: this.score,
...
// 从本地恢复游戏状态
loadGameState() {
    const stateStr = localStorage.getItem('gameState2048');
    if (!stateStr) return false; // 没有存储内容就跳过
    ...
    try {
        ...
        this.won = state.won;
        this.keepPlayingAfterWin = state.keepPlayingAfterWin;

        // 清空当前 DOM 方块容器
        this.tileContainer.innerHTML = '';
        // 根据 grid 中的值重新渲染 DOM 元素
        for (let x = 0; x < this.gridSize; x++) {
            for (let y = 0; y < this.gridSize; y++) {
                const tile = this.grid[x][y];
                if (tile) {
                    this.addTile(tile, false); // false 表示不要新生成动画
                }
            }
        }

        this.updateScore();      // 恢复分数显示
        this.updateGameState();  // 恢复胜利/失败显示
        return true;
    } catch (e) {
    ...
    getPositionFromCoords(x, y) {
        ...
        return {
  left: padding + (cellSize + gap) * y,
  top: padding + (cellSize + gap) * x
};
